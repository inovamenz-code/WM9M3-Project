...
public:
	...
	void init(Matrix ProjectionMatrix, int screenwidth, int screenheight)
	{
		...
		// RenderingAlgorithms p26: unit-distance film width and height, including aspect ratio.
	...
	Ray generateRay(float x, float y)
	{
		...
		Vec3 clipPoint(ndcX, ndcY, 1.0f);
		Vec3 cameraSpace = inverseProjectionMatrix.mulPointAndPerspectiveDivide(clipPoint);
		Vec3 dir = camera.mulVec(cameraSpace).normalize();
...
public:
	...
	// Pick one light uniformly from the list. pmf is the probability of that choice.
	// 从光源列表里均匀挑一个，pmf 是选中它的概率。
	Light* sampleLight(Sampler* sampler, float& pmf)
	{
		...
		int index = (int)(sampler->next() * (float)lights.size());
		// next() can round up to exactly 1.0f, which would index past the end.
		// next() 有可能四舍五入成 1.0f，那样会越界。
		if (index >= (int)lights.size())
		{
			index = (int)lights.size() - 1;
		}
